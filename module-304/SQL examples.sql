...
where e.office_id = o.id
    ...
    select * from oders where id = 10103; -- this only returns a single record
    -- however when we join  against the order table the order date is duplicated because each line time in the order details was essentially ordered on the same day
    ...
    select sum(quantity_ordered * price_each) as total_order_cost,
        ...
        min(quantity_ordered * price_each) as average_line_item_cost,
    ...
    -- I want to see the total order cost of each order for all orders
    select * from orderdetails;
    ...
    -- we also limited our columns in the result set so the data was a bit more consumable by a human brain
    ...
    -- we added a second column to the order by so we could see the products in each order by the order side
    ...
    -- now lets make this one more step complex. I only want to see the orders over 50k
    -- by adding having it allows us to filter the aggregate results and we cannot do this is the where clause because
    -- the aggregate result has not yet been calculated at the time the database is considering the where clause
    	select c.id, c.customer_name, o.id as order_id, sum(quantity_ordered * price_each) as total_order_cost
    from customers c, orders o, orderdetails od
    where c.id = o.customer_id
		and o.id = od.order_id
